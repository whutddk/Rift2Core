...
trait VDecode32{ this: DecodeBase =>
  if( hasVector ) {
    ...
    info.lsuIsa.vle       := ( x === BitPat("b????00?00000?????????????0000111") )
    info.lsuIsa.vlm       := ( x === BitPat("b????00?01011?????????????0000111") )
    info.lsuIsa.vlse      := ( x === BitPat("b????10???????????????????0000111") )
    info.lsuIsa.vloxei    := ( x === BitPat("b?????1???????????????????0000111") )
    info.lsuIsa.vleNff    := ( x === BitPat("b????00?10000?????????????0000111") )
    info.lsuIsa.vlNreN    := ( x === BitPat("b????00?01000?????????????0000111") )

    info.lsuIsa.vse       := ( x === BitPat("b????00?00000?????????????0100111") )
    info.lsuIsa.vsm       := ( x === BitPat("b????00?01011?????????????0100111") )
    info.lsuIsa.vsse      := ( x === BitPat("b????10???????????????????0100111") )
    info.lsuIsa.vsoxei    := ( x === BitPat("b?????1???????????????????0100111") )
    info.lsuIsa.vsNr      := ( x === BitPat("b????00?01000?????????????0100111") )
