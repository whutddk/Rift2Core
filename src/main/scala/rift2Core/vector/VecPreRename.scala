...
abstract class VecPreRenameBase()(implicit p: Parameters) extends RiftModule {
  ...
  val vlsNFSel    = Wire( Vec( 8, UInt(3.W)) )
...
trait VecPreRenameVlsMicInstr{ this: VecPreRenameBase =>
  ...
  for( i <- 0 until 8 ) {
    ...
    vlsMicInstr(i).param.raw.rs3 := 
      Mux1H( Seq(
        ( vSplitReq.lsuIsa.isVS3 ) -> (vSplitReq.param.raw.rs3 + i.U ), //VSTORE
      ))

    vlsMicInstr(i).param.raw.rd0 := 
      Mux1H( Seq(
        ( vSplitReq.lsuIsa.isVLoad  ) -> (vSplitReq.param.raw.rd0 + i.U ),
    ...
    vlsMicInstr(i).vAttach.get.tEleIdx   :=
      Mux1H(Seq(
