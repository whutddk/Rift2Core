...
trait AXI_mst_evict extends TLC_base {
  ...
  val info_mstEvict_address =
    Mux1H(Seq(
      is_mstReleaseData_StateOn  -> (info_slvAcquire_address & (Fill(64, 1.U) << addr_lsb)),
  ...
  when( ~is_mstEvict_stateOn_StateOn & is_mstReleaseData_allowen ) {
    info_mstProbeData_address := info_mstRecProbe_address
  } .elsewhen( is_mstReleaseData_StateOn & mst_chn_w.fire & ~is_mstProbeData_addrend ) {
    info_mstProbeData_address := info_mstProbeData_address + (1.U << bus_lsb)
  }

  when( ~is_mstEvict_stateOn_StateOn & is_mstProbeAckData_allowen ) {
    info_mstReleaseData_address := info_slvAcquire_address & (Fill(64, 1.U) << addr_lsb)
  } .elsewhen( is_mstProbeAckData_StateOn & mst_chn_w.fire & ~is_mstReleaseData_addrend ) {
    info_mstReleaseData_address := info_mstReleaseData_address + (1.U << bus_lsb)
  }


  when( ~is_mstEvict_stateOn_StateOn ) {
    when( is_mstReleaseData_allowen ) { is_mstReleaseData_Waiting := false.B; is_mstReleaseData_StateOn := true.B }
    .elsewhen( is_mstProbeAckData_allowen ) { is_mstProbeAckData_Waiting := false.B; is_mstProbeAckData_StateOn := true.B }
  }


  val is_mstEvict_stateOn_Waiting = RegInit(false.B)
  val is_mstEvict_stateOn_StateOn = RegInit(false.B)
  // val is_mstEvict_stateOn_allowen = Wire(Bool())
  ...
  mstEvict_state_dnxt := 
    Mux1H(Seq(
      ( mstEvict_state_qout === 0.U ) -> Mux( is_mstEvict_stateOn_StateOn, 1.U, 0.U),
  ...
  when( mstEvict_state_qout === 2.U & mstEvict_state_dnxt === 0.U ) {
    when( is_mstReleaseData_StateOn ) { is_mstReleaseData_StateOn := false.B }
    when( is_mstProbeAckData_StateOn ) { is_mstProbeAckData_StateOn := false.B }
  }
  ...
  mst_chn_aw.bits.addr := 
    Mux1H(Seq(
      ...
      is_mstReleaseData_StateOn  -> info_mstReleaseData_address
    ))
  ...
  mst_chn_aw.bits.len :=
    Mux1H(Seq(
      ...
      is_mstReleaseData_StateOn  -> (dw/bus_w-1).U
    ))
  ...
  mst_chn_w.bits.data :=
    Mux1H(Seq(
      ...
      is_mstReleaseData_StateOn  -> cache_dat.dat_addr_r(info_slvAcquire_cb)
    ))

  mst_chn_w.bits.last :=
    Mux1H(Seq(
      is_mstProbeAckData_StateOn -> is_mstProbeData_addrend,
      is_mstReleaseData_StateOn  -> is_mstReleaseData_addrend      
    ))
