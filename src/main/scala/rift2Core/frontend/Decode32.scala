...
class Decode32 (x:UInt, pc: UInt) {
  ...
  info.alu_isa.slti        := ( x === BitPat("b?????????????????010?????0010011") ) & x(11,7) =/= 0.U
  ...
  info.is_access_fault     := ( x === BitPat("b00000000000000000010000000010011") )
  info.is_paging_fault     := ( x === BitPat("b00000000000000010010000000010011") )
}
