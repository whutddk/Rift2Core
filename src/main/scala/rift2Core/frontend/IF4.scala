...
abstract class IF4Base()(implicit p: Parameters) extends IFetchModule {
  val io = IO(new Bundle{
    ...
    val tageResp = Vec(rnChn, Flipped(Decoupled(Vec(6, new TageTableResp_Bundle ))))

    val if4_resp = Vec(rnChn, Decoupled(new IF4_Bundle))

    val if4_update_ghist = Vec(rnChn, Valid(new Ghist_reflash_Bundle))
  ...
  val bftq = Module(new MultiPortFifo( dw = new Branch_FTarget_Bundle, (if(!isMinArea) 4 else 1), rnChn, 1 ))
  val jftq = Module(new MultiPortFifo( dw = new Jump_FTarget_Bundle,   (if(!isMinArea) 4 else 1), rnChn, 1 ))


  val bRePort = Module(new RePort( new Branch_FTarget_Bundle, port = rnChn) )
  val jRePort = Module(new RePort( new Jump_FTarget_Bundle,   port = rnChn) )
...
trait IF4_Predict{ this: IF4Base =>
  val redirectTarget = Wire(Vec(rnChn, UInt(64.W)))
  val isRedirect = Wire(Vec(rnChn, Bool()))

  val isDisAgreeWithIF1 = Wire(Vec(rnChn, Bool()))
  val isIf4Redirect = Wire(Vec(rnChn, Bool()))

  val is_bTaken = for ( i <- 0 until rnChn ) yield {
    if (!isMinArea) {
  ...
  val jalr_pc = for( i <- 0 until rnChn ) yield {
    extVaddr( Mux( is_return(i) & ras.io.deq.valid, ras.io.deq.bits.target, btb_decode(i).target), vlen )
  }
  //ignore ras-pop-valid
  ras.io.deq.ready := ( 0 until rnChn ).map{ i => (is_return(i) & io.if4_req(i).fire)}.reduce(_|_)

  ras.io.enq.valid := ( 0 until rnChn ).map{ i =>
    is_call(i) & io.if4_req(i).fire
  }.reduce(_|_)

  ras.io.enq.bits.target := MuxCase( 0.U, (0 until rnChn).map{ i => 
  ...
  io.if4Redirect.bits.target := 
    MuxCase(0.U, (0 until rnChn).map{ i =>
