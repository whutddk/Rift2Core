...
object Decode16 {
  def apply(x:UInt, pc: UInt, hasFpu: Boolean): Info_instruction = {
    ...
    def c_fld      = if(hasFpu) ( x === BitPat("b????????????????001???????????00") ) else false.B
    def c_fsd      = if(hasFpu) ( x === BitPat("b????????????????101???????????00") ) else false.B
    def c_fldsp    = if(hasFpu) ( x === BitPat("b????????????????001???????????10") ) else false.B
    def c_fsdsp    = if(hasFpu) ( x === BitPat("b????????????????101???????????10") ) else false.B
