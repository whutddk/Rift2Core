...
class iss_readOp_info(dp: Int) extends Bundle {
  val reg = Output(new Register_source(dp))
  val dat = Input(new Operation_source)
}


class RegFiles(dp: Int=64, rn_chn: Int = 2, rop_chn: Int=4, wb_chn: Int = 6, cmm_chn: Int = 2) extends Module{
  val io = IO( new Bundle{
    ...
    val iss_readOp = Vec(rop_chn, Flipped( Vec(3, new iss_readOp_info)) )
    /** writeBack request from exeUnit */
    val exe_writeBack = Vec(wb_chn, Flipped(new ValidIO(new WriteBack_info)))
