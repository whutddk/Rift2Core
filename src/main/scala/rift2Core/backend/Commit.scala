  ...
  for ( i <- 0 until cmChn ) {
    ...
    .otherwise {
      when(
        ...
        when(
          (io.rod(i).bits.is_csr & ~io.csrCmm(i).isWroteback ) || //Mux(io.rod(i).bits.csrw === 0.U, ~csrExe(i).valid, ~io.csrCmm(i).isWroteback) ) 
...
with CommitDiff{
  ...
  ( 0 until cmChn ).map{ i => {
    ...
    cmm_state(i).csrExe  := io.csrOp(i) //Mux( io.rod(i).bits.csrw === 0.U, csrExe(i).bits, io.csrOp(i))
