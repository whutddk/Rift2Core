    ...
    lsuIssInfo(i).param.dat.op1 := 
      Mux1H(Seq(
        ...
        (bufInfo(i).lsuIsa.is_lrsc | bufInfo(i).lsuIsa.is_amo)  -> postBufOperator(i)(1),
              ...
              (bufInfo(i).lsuIsa.isIndex      & bufInfo(i).vAttach.get.vsew === "b10".U ) -> (postBufOperator(i)(1).asSInt + postBufOperator(i)(2).asSInt                                     + ( bufInfo(i).vAttach.get.nfSel << 2 ).asSInt ).asUInt(),//vs2 -> vop2 
              (bufInfo(i).lsuIsa.isIndex      & bufInfo(i).vAttach.get.vsew === "b11".U ) -> (postBufOperator(i)(1).asSInt + postBufOperator(i)(2).asSInt                                     + ( bufInfo(i).vAttach.get.nfSel << 3 ).asSInt ).asUInt(),//vs2 -> vop2 
      )} else { Seq() })
