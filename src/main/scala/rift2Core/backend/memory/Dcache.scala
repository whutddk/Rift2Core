...
trait DcacheScoreBoard { this: DcacheBase =>
  ...
  val sbBuff = RegInit(VecInit(Seq.fill(sbEntry)(0.U.asTypeOf(new Info_cache_s0s1))))
  ...
  when( io.deq.fire ) {
    sbBuff (io.deq.bits.chk_idx) := 0.U.asTypeOf( new Info_cache_s0s1 )
    sbValid(io.deq.bits.chk_idx) := false.B
  }
  ...
  for ( i <- 0 until sbEntry ) yield {
    when( sbValid(i) === true.B ) {
      assert( sbBuff.count( (x: Info_cache_s0s1) => (x.paddr === sbBuff(i).paddr) ) === 1.U )
    }
  }
...
class Dcache(edge: TLEdgeOut, id: Int)(implicit p: Parameters) extends DcacheBase(edge, id) with DcacheScoreBoard{
  ...
  op_arb.io.in(1).bits.chk_idx := sbIdx //override
  ...
  stage.io.enq.valid := rd_arb.io.out.valid
  stage.io.enq.bits  := rd_arb.io.out.bits
